...
var _ = Context(
	"BeLessThanMatcher", func() {

		Describe(
			"asserting on nil", func() {
				success, err := (&matchers.BeLessThanMatcher{Expected: decimal.NewFromInt(1)}).Match(nil)
		...
		DescribeTable(
			"passed matchers", func(expected, actual interface{}) {
				Ω((&matchers.BeLessThanMatcher{Expected: expected}).Match(actual)).Should(BeTrue())
			},
		...
		DescribeTable(
			"passed matchers (Not)", func(expected, actual interface{}) {
				Ω((&matchers.BeLessThanMatcher{Expected: expected}).Match(actual)).ShouldNot(BeTrue())
			},
		...
		DescribeTable(
			...
			func(expected, actual interface{}, expectedValue string) {
				matcher := &matchers.BeLessThanMatcher{Expected: expected}
			...
			"asserting failed convert", func() {
				DescribeTable(
					"actual on failure string", func(value string) {
						success, err := (&matchers.BeLessThanMatcher{Expected: decimal.Decimal{}}).Match(value)
				...
				DescribeTable(
					"expected on failure string", func(value string) {
						success, err := (&matchers.BeLessThanMatcher{Expected: value}).Match(decimal.Decimal{})
						Ω(success).To(BeFalse())
						Ω(err).Should(MatchError(gdecimalErrors.ErrFailedConvertStringToDecimal))
					},
					converterTestData.AssertingOnStringFailureEntries,
